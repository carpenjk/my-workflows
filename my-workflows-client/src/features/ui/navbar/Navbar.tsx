...
const Navbar = ({className}: Props) => {
  ...
  return (
    <>
      <div className={twMerge(`fixed bottom-0 md:relative w-screen md:h-screen md:pl-4 md:pt-28 md:pr-8 md:flex 
        ...
        <nav className='relative w-full'>
          ...
          <button onClick={()=> setIsCollapsed((prev)=> !prev)} className='absolute hidden p-1 border rounded-md md:flex text-text-normal dark:text-dk-text-normal slate-400 w-7 h-7 -right-11 top-3 hover:animate-bounce-once border-primary-6 dark:border-dk-primary-6'><ArrowsRightLeftIcon/></button>
        </nav>
      </div> 
  </>
  );
